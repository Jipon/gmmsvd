    ...
    def __init__(self, dim = None, ncomps = None, data = None,  method = None, filename = None, params = None):
        ...
        if not filename is None:  # load from file
            ...
            if method is "uniform":
                ...
                mus = pr.sample(list(data),ncomps)
                ...
                clusters = [[] for i in range(int(ncomps))]
                #print(clusters)
                ...
                self.priors = np.ones(int(ncomps), dtype="double") / np.array([len(c) for c in clusters])
